 ...
 *
 */
//TODO 该处理器主要工作：将事务请求记录到磁盘日志中；  为了效率，批量io；写完磁盘后才会交给下一个处理器；
//TODO 该处理器leader、follower和observer都会调用，不同角色不同操作
//TODO 1.leader：将请求写入磁盘，并将其转发到AckRequestProcessor，后者将ack发回给自己。
// *   2.follower：将请求写入磁盘，并将请求转发到SendAckRequestProcessor，后者将数据包发送到领导者。SendAckRequestProcessor是可刷新的，
// *                这使我们能够强制执行将数据包推送给领导者。
// *   3.observe：将提交的请求同步到磁盘（作为INFORM数据包接收）。它永远不会将确认发送回给领导者，因此nextProcessor将为空。
